...
Q: 在workInProgress树构建完成后，fiberRootNode的current指向了新fiber树，那旧的fiber树怎么处理?
A: 旧树中，被复用的节点保存了下来，未被复用的节点被删除了，浏览器会将删除的节点内存收回。因为新fiber树的alternate属性指向了被复用的节点，因此被复用的节点会被保留下来。更新阶段，新旧节点只是互换，但不新创建出节点。
